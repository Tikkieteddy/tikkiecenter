...
Tikkie Trip เป็นเว็บแอปสำหรับวางแผนเที่ยวด้วยรถ EV ภายใน repository `Tikkieteddy/tikkiecenter` โดยแยกเป็น Next.js application อิสระในโฟลเดอร์ `trip/` เพื่อไม่กระทบ Tikkie Center เดิมที่อยู่ repository root
...
สร้างไฟล์ `trip/.env.local` จาก `trip/.env.example`
...
6. จำกัด HTTP referrer เช่น `https://tikkiecenter-trip.vercel.app/*` และ localhost ที่ใช้พัฒนา
...
ให้สร้าง Vercel Project ใหม่ชื่อ `tikkiecenter-trip` โดยใช้ repository เดิม `Tikkieteddy/tikkiecenter`

## วิธีเชื่อม repo เดียวกับ Vercel Project ใหม่

1. เข้า Vercel Dashboard
2. Add New Project
3. เลือก GitHub repository `Tikkieteddy/tikkiecenter`
4. อย่าสร้าง GitHub repository ใหม่

## วิธีตั้ง Root Directory เป็น `trip`
...
เว็บไซต์หลักเดิมยังใช้ Root Directory เป็น repository root เหมือนเดิม

## วิธีตั้งชื่อ Project เป็น `tikkiecenter-trip`
...
```text
https://tikkiecenter-trip.vercel.app
```

## วิธีเพิ่ม Environment Variables

เพิ่มใน Vercel Project `tikkiecenter-trip`
